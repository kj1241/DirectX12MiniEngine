...
public:
    ...
    struct FontChar
    {
        ...
        float textWidth; // 텍스트 문자 넓이  16byte 
        float textHeight; // 텍스트 문자 높이  20byte
        ...
        float page; // 페이지 수를 가르키는거
        float channel; // 채널
    };
    BMFontUI();
    ~BMFontUI();

    float GetKerning(WCHAR first, WCHAR second);     // 두문자를 사용하는 커닝의 양을 반환
    FontChar* GetChar(WCHAR c);     // 주어진 fonchar를 반환
    ...
    ComPtr<ID3D12Resource> textureBuffer; // 글꼴 텍스쳐 리소스

private:
    std::wstring name; //폰트 이름
    std::wstring fontImage;
    int size; //글꼴크기 라인높이는 (1,0)기반
    float lineHeight=0; //다음줄로 얼마나 이동해야 되는지
    float baseHeight=0; //모든 문자 높이가 정규화
    int textureWidth=0; //글꼴 텍스쳐 너비
    int textureHeight=0; //글꼴 텍스쳐 높이
    int numCharacters =0; //글꼴 문자수
    FontChar* CharList = {}; // 문자 목록
    int numKernings =0; // 커닝 수
